...
<div class="container-fluid">
  <div class="row-fluid">
	...
	$select=mysql_query("select * from servicer where id={$id}");		//定义查询语句
	$row=mysql_fetch_array($select);												//循环输出查询内容
	

     echo " <form id='tab' method='post' action='script/staff_updates.php?id=$id' autocomplete='off'>"
?>
          <label>员工姓名</label>
          ...
          <label>手机号</label>
          <input type="text" name="user_phone" value="<?php echo $row['phonenumber'];?>" class="input-xlarge" required pattern="\d{11}">
          <label>学校</label>
